...
for i = 1:length(brainareas)
    ...
    if ~strcmp(brainarea, 'STN') &&~strcmp(brainarea, 'GP')
        ...
        plotPSD_comp_1chn(psd_normal, psd_mild, F_pxx, plotF_AOI, savefolder, brainarea, animal, ylimits, psd_moderate)
        
    else
        % if DBS case, pxxs_allfiles.GP, pxxs.GP: nfs * nchns * nsegs
        plotPSD_comp_multichns(psd_normal, psd_mild, F_pxx, plotF_AOI, savefolder, brainarea, animal, ylimits, psd_moderate)
    end
...
function plotPSD_comp_1chn(psd_normal, psd_mild, F_all, plotF_AOI, savefolder, brainarea, animal, ylimits, varargin)
...
if exist('psd_moderate_FAOI', 'var')
...
if exist('psd_moderate_high', 'var') && exist('psd_moderate_low', 'var') && exist('psd_moderate_mean', 'var')
...
if exist('psd_moderate_high', 'var') && exist('psd_moderate_low', 'var')
...
if exist('psd_moderate_mean', 'var')
...
function plotPSD_comp_multichns(psd_normal, psd_mild, F_all, plotF_AOI, savefolder, brainarea, animal, ylimits, varargin)
%%  plot the psd comparison of normal and mild
%
%   Inputs:
...
for chni = 1:nchns
    ...
    if exist('psd_allsegs_moderate', 'var')
