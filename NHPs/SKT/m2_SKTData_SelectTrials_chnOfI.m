function m2_SKTData_SelectTrials_chnOfI(inputanimal, varargin)
%  plot spectrogram of each trial and manually select trials
%
%   Usage:
%            m2_SKTData_SelectTrials_chnOfI('Kitty', 'fi_str', 1, 'fi_end', 8, 'autoSaveMode', 'y');
%
%   Input:
%       Name-Value: 
%           'animal'
...
%
%   Note:
%       the trials marked with bad trials are unchecked automatically
%
...
[t_minmax_reach_normal, ~, t_minmax_reach_mild, ~, t_minmax_reach_moderate, ~] = ...
...
while(filei <=  fi_end)
    ...
    if ~strcmpi(autoSaveMode, 'y')
        ...
        if isempty(reply) || lower(reply) ~= 'n'
            ...
            saveas(gcf, oneday_spectrogram_img, saveimg_format);
