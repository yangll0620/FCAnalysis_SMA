function m0_SKTData_extract(animal, varargin)
%% extract the STK trials [-t_bef+t_targetonset t_mouth+t_aft] from processed folder in server 
%
%	Inputs:
%
%		animal
%
%       Name-Value: 
%           't_bef': the time before target on (default: t_bef = 3)  
%           't_aft': the time after mouth (default: t_aft = 1)
%   
%   Steps:
%       1. extract trials from processed folder in server 
%           a. trial length = t_targetonset2mouth + t_bef + t_aft
%
...
% parse varargin
p = inputParser;
addParameter(p, 't_bef', 3, @(x)isscalar(x)&&isnumeric(x));
...
if strcmpi(animal, 'Kitty')
    ...
    t_SKT = readtable(xlsxfile_master, opts);
...
for i = 1:nrecords
    ...
    % date of exp, bktdt
    if any(strcmp(t_SKT.Properties.VariableNames, 'Date'))
        dateofexp = datenum(t_SKT.Date(i), strformat_date_master);
    else
        if any(strcmp(t_SKT.Properties.VariableNames, 'OutputFolderName'))
            ...
            disp(['dateofexp can not be parsed!'])
            break;
        end
    end
    ...
    if strcmpi(animal, 'Kitty') && strcmpi(pdcond, 'normal')
        [lfptrial_cortical, lfptrial_dbs, mask_goodreach, mask_goodreturn, fs_lfp, T_idxevent_lfp, fs_ma, T_idxevent_ma, smoothWspeed_trial, Wpos_smooth_trial, Wrist_smooth_trial, T_dbsChn] = ...
            extractlfptrial_cell_normalCOT_Kitty(onedaypath, tdtbk, 't_bef', t_bef, 't_aft', t_aft);
    else
        [lfptrial_cortical, lfptrial_dbs, mask_goodreach, mask_goodreturn, fs_lfp, T_idxevent_lfp, fs_ma, T_idxevent_ma, smoothWspeed_trial, Wpos_smooth_trial, Wrist_smooth_trial, T_dbsChn] = ...
            extractlfptrial_cell(onedaypath, tdtbk, 't_bef', t_bef, 't_aft', t_aft);
    end
...
function [lfptrial_cortical, lfptrial_dbs, mask_goodreach, mask_goodreturn, fs_lfp, T_idxevent_lfp, fs_ma, T_idxevent_ma, smoothWspeed_trial, Wpos_smooth_trial, Wrist_smooth_trial, T_dbsChn] = extractlfptrial_cell(onedaypath, tdtblock, varargin)
...
%   tdtblock = 3
%
%   [lfptrial_cortical, lfptrial_dbs, fs, idxeventtbl, chantbl_dbs] = extractlfptrial_(onedaypath, tdtblock, 't_bef', 5, 't_aft', 1);
%
%  Inputs:
...
%
%  Outputs:
%       lfptrial_cortical: lfp trials of cortical/subcortical channels, cell: ntrials * 1, in each cell ntemp * nchns
%
%        lfptrial_dbs: lfp trials of dbs channels, cell: ntrials * 1, in each cell ntemp * nchns, 1-8: STN, 9-16:GP
...
%
%  More Description:
%       trial length = each trial length + t_bef + t_aft
%       t_bef: the time before target on (default: t_bef = 3)
%       t_aft: the time after mouth (default: t_aft = 1)
...
mafilestruct2 = dir(fullfile(mafolder, '*SingleTargetKluver_Analyze2.mat'));
if length(mafilestruct2) == 1 % load SingleTargetKluverMAData from  *SingleTargetKluver_Analyze2.mat
    % load SingleTargetKluverMAData from  *SingleTargetKluver_Analyze2.mat
    load(fullfile(mafolder, mafilestruct2.name), 'SingleTargetKluverMAData');
else
    mafilestruct1 = dir(fullfile(mafolder, '*SingleTargetKluver_Analyze1.mat'));
    if length(mafilestruct1) == 1 
        load(fullfile(mafolder, mafilestruct1.name), 'Analyze');
        SingleTargetKluverMAData = Analyze;
        clear Analyze
    else
        disp([mafolder 'has no *Analyzed1.mat and *Analyzed2.mat skip!'])
...
for triali = 1:n_trial
    idx_str = idx_strs(triali);
    idx_end = idx_ends(triali);
    smoothWspeed_trial{triali} = SingleTargetKluverMAData.smoothWspeed(idx_str : idx_end, :);
    Wpos_smooth_trial{triali} = SingleTargetKluverMAData.Wpos_smooth(idx_str : idx_end, :);
    Wrist_smooth_trial{triali} = SingleTargetKluverMAData.Wrist_smooth(idx_str : idx_end, :);
...
function [lfptrial_cortical, lfptrial_dbs, mask_goodreach, mask_goodreturn, fs_lfp, T_idxevent_lfp, fs_ma, T_idxevent_ma, smoothWspeed_trial, Wpos_smooth_trial, Wrist_smooth_trial, T_dbsChn] = ...
    extractlfptrial_cell_normalCOT_Kitty(onedaypath, tdtbk, varargin)
% extractlfptrial extract trials for LFP data
%
