function m2_restData_PSD_fs1000()
...
% input folder
inputfolder = fullfile(codecorresParentfolder, 'm1_restData_rmChns_avgArea_fs1000');
...
file_psdall = fullfile(savefolder, [savefilename_prefix '_allsegs_normalmildmoderate' num2str(plotF_AOI(1)) '-' num2str(plotF_AOI(2)) 'Hz.mat']);
...
if ~exist(file_psdall, 'file')% not exist
    ...
    if ~isequal(F_pxx_normal, F_pxx_moderate)
        disp('F_pxx_normal, and F_pxx_moderate not equal');
        
        return;
    else
        F_pxx = F_pxx_normal;
        
        save(file_psdall, 'pxxs_allfiles_normal', 'pxxs_allfiles_moderate', 'F_pxx');
        clear F_pxx_normal  F_pxx_moderate
    end
    
else
    load(file_psdall, 'pxxs_allfiles_normal', 'pxxs_allfiles_moderate', 'F_pxx');
end

%%%  plot  %%%
brainareas = fieldnames(pxxs_allfiles_normal);
...
clear psd_normal_high psd_normal_low psd_normal_mean psd_moderate_high psd_moderate_low psd_moderate_mean
