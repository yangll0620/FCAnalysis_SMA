...
if(segVFolder)
    ...
    inputfolder_SKTiCoh = fullfile(codecorresParentfolder, 'm4_imCohUsingFFT_EventPhase_goodReach');
else
...
for ci = 1 : length(cond_cell)
    ...
    for ei = 1: length(EventPhases)
        ...
        if(~(isequal(round(vars_SKTiCoh.f_selected,2), round(f_selected,2)) && isequal(vars_SKTiCoh.chnPairNames, chnPairNames) && vars_SKTiCoh.ntrials == ntrials))
            disp('not equal in SKTiCoh')
        end
