function m5_imCohChanges_reachFreeze_after2late(varargin)
% plot cicoh changes of after200ms ReachFreeze compared to late ReachFreeze
% relative to premove and early reach
%
%   Example Usage:
%           m5_imCohChanges_reachFreeze_late2after('shuffleN_psedoTest', 500, 'newRun', true)
%   
%   Input:
%       Name-Value:
...
baseSubFreezeType = 'lateFreeze';
compSubFreezeType = 'afterFreeze200ms';


ciCoh_Changes_file = fullfile(savefolder, 'ciCohs-Changes-reachFreeze-after2late.mat');
if (~exist(ciCoh_Changes_file, 'file') || newRun)

    [~, ciCoh_base, f_selected]= ciCoh_trialDeltaPhi(lfpsegs_Freeze.(freezeType).(baseSubFreezeType), fs, f_AOI);
    [~, ciCoh_comp, ~]= ciCoh_trialDeltaPhi(lfpsegs_Freeze.(freezeType).(compSubFreezeType), fs, f_AOI);

    ciCohChanges = ciCoh_comp - ciCoh_base;

    save(ciCoh_Changes_file, 'lfpsegs_Freeze', 'ciCohChanges', 'f_selected', 'fs', 'f_AOI','T_chnsarea');

    clear('lfpsegs_Freeze', 'ciCohChanges', 'f_selected', 'fs', 'f_AOI','T_chnsarea');
    clear ciCoh_base f_selected ciCoh_comp
end


% psedoCicohChanges
load(ciCoh_Changes_file, 'lfpsegs_Freeze',  'fs', 'f_AOI')
lfpsegs_base = lfpsegs_Freeze.(freezeType).(baseSubFreezeType);
lfpseg_comp = lfpsegs_Freeze.(freezeType).(compSubFreezeType);
psedorCiCohChanges_after2late_extract_save(shuffleN_psedoTest, lfpseg_comp, lfpsegs_base, fs, f_AOI, ciCoh_Changes_file);
clear('lfpsegs_Freeze', 'fs', 'f_AOI')
clear lfpsegs_base lfpseg_comp
...
plot_ciCohHistogram2(sigciCohChanges_flatten, chnPairNames, f_selected, 'Reach Freeze after-late', 'histClim', [-1 1],...
    ...
    'fig', ifig);
subfilename = 'ciCohChanges-ReachFreeze-after2late'; 
print(ifig, fullfile(savefolder, subfilename), '-painters', '-depsc')
print(ifig, fullfile(savefolder, subfilename), '-dpng', '-r1000')
close(ifig)


function psedorCiCohChanges_after2late_extract_save(suffi_end, lfptrials, lfptrials_base, fs, f_AOI, ciCohChangesfile)
%
%   Inputs:
%       suffi_end
...
for si = shuffi_str : suffi_end
    ...
    if(mod(si, 100) == 0)
        disp([' Reach Phase after2late pesdo ciCoh Changes test ' num2str(si)])
