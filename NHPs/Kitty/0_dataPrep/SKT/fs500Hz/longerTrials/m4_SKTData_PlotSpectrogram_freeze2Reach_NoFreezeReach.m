...
plot_spectrogramMA_acrossTrials(lfptrials, matrials, fs_ma, T_chnsarea, tdur_trial, fs_lfp, animal, pdcond, align2, savefolder, 't_plot', t_plot);
...
function plot_spectrogramMA_acrossTrials(lfptrials, matrials, fs_ma, T_chnsarea, tdur_trial, fs_lfp, animal, pdcond, align2, savefolder, varargin)
...
for chi = 1 : size(lfptrials, 1)
    ...
    if ~isempty(t_plot)
        idx_t = (times >= t_plot(1) &  times <=t_plot(2));
...
for chi = 1 : height(T_chnsarea)
    ...
    title(ax2, ['NoFreeze2Reach-' brainarea])
    savefile = fullfile(savefolder, [ 'NoFreeze2Reach-MA-spectrogram-' brainarea]);
    print(gcf, savefile, '-painters', '-depsc')
    print(gcf, savefile, '-dpng', '-r1000')
    close all
    clear ax1 ax2
