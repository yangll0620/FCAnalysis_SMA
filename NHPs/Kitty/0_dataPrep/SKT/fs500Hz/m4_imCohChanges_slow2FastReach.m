function m4_imCohChanges_slow2FastReach(varargin)
% plot cicoh changes of slow trials compared to the fast trials
%
%   Example Usage:
%           m4_fs500Hz_imCohChanges_slowFastReach('t_slowFastTheshold', 1.5, 'shuffleN_psedoTest', 500, 'newRun', true)
%   
%   Input:
%
%       Name-Value:
...
if (~exist(ciCoh_Changes_file, 'file') || newRun)
    ...
    for ei = 1 : length(ePhases)
        ...
        ciCohChangs_SF.(ePhase) = ciCoh_slow - ciCoh_fast;
    ...
    save(ciCoh_Changes_file, 'ciCohs', 'f_selected', 'ciCohChangs_SF', '-append');

    clear fs T_chnsarea 
    clear align2name align2 t_AOI

end
clear('lfptrials', 'fs', 'T_chnsarea', 'f_AOI', 'ciCohs', 'ciCohChangs_SF', 'f_selected')
...
for ei = 1 : length(ePhases)
    ...
    psedoSlowFastciCohChanges_extract_save(shuffleN_psedoTest, lfptrials_slow, lfptrials_fast, fs, f_AOI, ciCoh_Changes_file,  ePhase);
...
% plot
load(ciCoh_Changes_file, 'ciCohChangs_SF', 'psedociCohChanges_SF', 'f_selected',  'T_chnsarea')
for ei = 1 : length(ePhases)
    ePhase = ePhases{ei};

    ciCohchange = ciCohChangs_SF.(ePhase);
    psedoiCohChange = psedociCohChanges_SF.(ePhase);
    ...
    plot_ciCohHistogram2(sigciCohChanges_flatten, chnPairNames, f_selected, [ePhase ' slowReach-fastReach'], 'histClim', [-1 1],...
        ...
        'fig', ifig);
    subfilename = ['ciCohChanges-' 'slow2FastReach-' ePhase]; % 'Jo-mild-Bnormal-preMove'
...
load(ciCohChangesfile, 'psedociCohChanges_SF');

if(~exist('psedociCohChanges_SF', 'var'))
    psedociCohChanges_SF = struct();
end

if (~isfield(psedociCohChanges_SF, ePhase))
    psedociCohChanges_SF.(ePhase) = [];
    shuffi_str = 1;
else
    shuffi_str = size(psedociCohChanges_SF.(ePhase), 4) + 1;
end
...
for si = shuffi_str : suffi_end
    ...
    psedociCohChanges_SF.(ePhase) = cat(4, psedociCohChanges_SF.(ePhase), psedoiCoh_comp - psedoiCoh_Base);
    
    if(mod(si, 100) == 0)
        disp([ePhase ' pesdo ciCoh Changes test ' num2str(si)])
        save(ciCohChangesfile, 'psedociCohChanges_SF', '-append');
    end
