function m5_imCohChanges_reachFreeze2ReachPhases(varargin)
...
ciCoh_Changes_file = fullfile(savefolder, 'ciCohs-Changes-reachFreeze2reachPhases.mat');
...
for ebi = 1 : length(eBasePhases)
    ...
    for fri = 1 : length(reachfreezeTypes)
        ...
        % psedo Cicoh
        psedoFreezeCiCoh_extract_save(shuffleN_psedoTest, lfpsegs, fs, f_AOI, ciCoh_Changes_file, 'ReachFreeze', subfreezeType);
...
for ebi = 1 : length(eBasePhases)
    ...
    for fri = 1 : length(reachfreezeTypes)
        ...
        ciCohchange = ciCohChanges.(['b_' eBasePhase]).ReachFreeze.(subfreezeType);
        psedoiCohChange = psedociCohChanges.(['b' eBasePhase]).ReachFreeze.(subfreezeType);
        ...
        plot_ciCohHistogram2(sigciCohChanges_flatten, chnPairNames, f_selected, ['reachFreeze ' subfreezeType '-' eBasePhase], 'histClim', [-1 1],...
            ...
            'fig', ifig);
        subfilename = ['reachFreeze2ReachPhase-' eBasePhase '-' subfreezeType]; 
...
function psedoFreezeReachCiCohChanges_extract_save(suffi_end, lfptrials, lfptrials_base, fs, f_AOI, ciCohChangesfile,  eBasePhase, freezeType, subfreezeType)
%
%   Inputs:
%       suffi_end
%       lfptrials: nchns * ntemp * ntrials(nsegs)
...
if ~isfield(psedociCohChanges, eBasePhase) || ~isfield(psedociCohChanges.(eBasePhase), freezeType) || ~isfield(psedociCohChanges.(eBasePhase).(freezeType), subfreezeType)
    psedociCohChanges.(eBasePhase).(freezeType).(subfreezeType) = [];
    shuffi_str = 1;
else
    shuffi_str = size(psedociCohChanges.(eBasePhase).(freezeType).(subfreezeType), 4) + 1;
end
...
for si = shuffi_str : suffi_end
    ...
    psedociCohChanges.(eBasePhase).(freezeType).(subfreezeType) = cat(4, psedociCohChanges.(eBasePhase).(freezeType).(subfreezeType), psedoiCoh_comp - psedoiCoh_Base);
    
    if(mod(si, 100) == 0)
        disp([eBasePhase '-' freezeType '-' subfreezeType ' pesdo ciCoh Changes test ' num2str(si)])
