...
folder_input_normal = fullfile(codecorresParentfolder, 'm0_normalCOTData_extract');
...
pdcond  = 'normal';
files = dir(fullfile(folder_input_normal, ['*_' pdcond '_*.mat']));
t_event = [];
for fi = 1: length(files)
    load(fullfile(folder_input_normal, files(fi).name), 'T_idxevent', 'fs');
...
pdcond  = 'moderate';
files = dir(fullfile(folder_input_moderate, ['*_' pdcond '_*.mat']));
t_event = [];
for fi = 1: length(files)
    load(fullfile(folder_input_moderate, files(fi).name), 'T_idxevent', 'fs');
...
ts = [t_normal; t_moderate];


% wilcoxon rank sum test
p1 = ranksum(t_normal, t_moderate);
...
coli = 1;
t_median = [median(tbl_normal{:,coli});  median(tbl_moderate{:,coli})];
t_10 = [quantile(tbl_normal{:,coli}, 0.1);  quantile(tbl_moderate{:,coli}, 0.1)];
t_25 = [quantile(tbl_normal{:,coli}, 0.25);  quantile(tbl_moderate{:,coli}, 0.25)];
t_75 = [quantile(tbl_normal{:,coli}, 0.75); quantile(tbl_moderate{:,coli}, 0.75)];
t_90 = [quantile(tbl_normal{:,coli}, 0.9);  quantile(tbl_moderate{:,coli}, 0.9)];
...
save(fullfile(savefolder, [animal '_SKTTime.mat']), 'tbl_normal', 'tbl_moderate', 'tbl_staReachT', 'tbl_staReturnT')