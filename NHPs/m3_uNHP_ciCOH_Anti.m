function m3_uNHP_ciCOH_Anti(animal, varargin)
%
%   Example Usage:
%           m3_uNHP_ciCOH_Anti('Jo', 'ei_str', 1, 'ci_str', 1, 'shuffleN_psedoTest', 200)
%           m3_uNHP_ciCOH_Anti('Kitty', 'ei_str', 1, 'ci_str', 1, 'shuffleN_psedoTest', 100)
%
%   Input:
%       Name-Value: 
%           animal
...
EventPhases = {'preMove'; 'Anticipated';'earlyReach';};
...
[t_minmax_reach_normal, t_minmax_return_normal, t_minmax_reach_mild, t_minmax_return_mild, t_minmax_reach_moderate, t_minmax_return_moderate] = ...
    goodSKTTrials_reachReturn_tcritiria(animal, 'codesavefolder', codesavefolder);
...
for ei = ei_str: ei_end
    ...
    for ci = ci_str : ci_end
        ...
        [align2, t_AOI, ~] = SKT_EventPhase_align2_tAOI_extract(event, animal, 'pdcond', pdcond, 'codesavefolder', codesavefolder);
        
        % load(and extract) ciCohfile
        ciCohfile = fullfile(savefolder, [ciCohfile_prefix  '_' pdcond '_' event '.mat']);
        ...
        if(~exist(ciCohfile, 'file') || newRun)
            ...
            save(ciCohfile, 'ciCoh', 'T_chnsarea', 'lfptrials', 'f_selected', 'fs', 'f_AOI');
            
            clear files t_minmax_reach
            clear ChnsOfI_mask
            clear('ciCoh', 'T_chnsarea', 'lfptrials', 'f_selected', 'fs');
        end
        ...
        if isempty(psedoVar) || psedoVar.size(4)< shuffleN_psedoTest
            load(ciCohfile, 'lfptrials', 'fs', 'f_AOI');
            psedociCoh_extract_save(shuffleN_psedoTest, lfptrials, fs, f_AOI, ciCohfile, codesavefolder);
            clear lfptrials fs 
        end
        
        clear pdcond align2 t_AOI ciCohfile
    end
end


function psedociCoh_extract_save(suffi_end, lfptrials, fs, f_AOI, ciCohfile, codesavefolder)
%
%   Inputs:
%       suffi_end
