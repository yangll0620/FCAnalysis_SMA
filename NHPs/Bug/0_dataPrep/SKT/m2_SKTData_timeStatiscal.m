...
%%% remove the trials with super longer/shorter reach and return
t_minreach = 0.3;
t_minreturn = 0.3;
t_maxreach = 1.5;
t_maxreturn = 1.5;
tbl_normal = tbl_normal(tbl_normal.t_reach>=t_minreach & tbl_normal.t_return>=t_minreturn & tbl_normal.t_reach<=t_maxreach & tbl_normal.t_return<=t_maxreturn,:);
tbl_mild = tbl_mild(tbl_mild.t_reach>=t_minreach & tbl_mild.t_return>=t_minreturn & tbl_mild.t_reach<=t_maxreach & tbl_mild.t_return<=t_maxreturn,:);
...
annotation(gcf,'textbox',...
    [0.25 0 0.2 0.2],...
    'String',['wilcoxon rank sum test', newline,  ['p = ' num2str(p1)]],...
    'LineStyle','none',...
    'HorizontalAlignment','right',...
    'FitBoxToText','off');
...
% mild
bi = 1;
text(boxes(bi).XData(3)* 1.01, boxes(bi).YData(3), [num2str(quantile(t_mild, 0.75) * 1000) ' ms'])
text(boxes(bi).XData(4)* 1.01, boxes(bi).YData(4), [num2str(quantile(t_mild, 0.25) * 1000) ' ms'])
% normal
bi = 2;
text(boxes(bi).XData(3)* 1.01, boxes(bi).YData(3), [num2str(quantile(t_normal, 0.75) * 1000) ' ms'])
text(boxes(bi).XData(4)* 1.01, boxes(bi).YData(4), [num2str(quantile(t_normal, 0.25) * 1000) ' ms'])
