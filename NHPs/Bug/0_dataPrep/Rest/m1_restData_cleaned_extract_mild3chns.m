function m1_restData_cleaned_extract_mild3chns()
%% extract cleaned restData for mild condition who only has 3 chns
%
...
inputfolder_moderate = fullfile(datafolder,animal, 'Recording', 'Processed', 'DataDatabase');
...
for fi = 1: nfiles
    ...
    [data_segments, segsIndex, therapy, fs] = segments_extract_fromAllSyncMat_3chns(file_allsync, arraychns_to_remove, thr_power, thr_eye_open, t0, min_comp_time, config_m1lf.freq_band);
...
function [data_segments, segsIndex, therapy, fs] = segments_extract_fromAllSyncMat_3chns(file_allsync, arraychns_to_remove, thr_power, thr_eye_open, t0, min_comp_time, freq_band)
...
idx = strfind(file_allsync, 'Block-');
onedaypath = file_allsync(1: idx -2);
blockstr = regexp(file_allsync, 'Block-[0-9]*','match');
tdtblock = str2num(blockstr{1}([length('Block-')+1:end]));
folder_LFP = fullfile(onedaypath, 'LFP', ['Block-' num2str(tdtblock)]);

nexfiles = dir(fullfile(folder_LFP, '*_LFPch1.nex'));
tmp = split(nexfiles(1).name, '1.nex');
nexfile_prefix = tmp{1};
chns = [1:96];
lfp_array = [];
for i = 1: length(chns)
    filename = [nexfile_prefix num2str(chns(i)) '.nex'];
    [nexlfp_LFP] = readNexFile(fullfile(folder_LFP, filename));
    
    % extract the number of the structure containing LFPchn* data
    name_list = extractfield(cell2mat(nexlfp_LFP.contvars),'name');
    i_lfp = find(contains(name_list, 'LFP')); % % i.e nexlfp_utah.contvars name == 'LFPch1', or 'MUAch1'
    
    lfp_array = cat(2, lfp_array, nexlfp_LFP.contvars{i_lfp,1}.data);
    
    clear filename nexlfp_LFP name_list i_lfp
end
if size(lfp_array, 1) ~= size(data.lfp_array, 1)
    disp('lfp length in nex not equal to length of data.lfp_array');
    disp(folder_LFP);
    return;
end

clear idx onedaypath blockstr tdtblock
