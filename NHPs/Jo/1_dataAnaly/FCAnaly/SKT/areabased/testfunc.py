...
def fc4drawing(chnAreas, fs, chnInf_file, lfptrials_normal = None, lfptrials_mild = None, lfptrials_moderate = None):
    ...
    if lfptrials_normal is not None and lfptrials_mild is not None and lfptrials_moderate is not None:
        ntrials_normal = lfptrials_normal.shape[2]
        ntrials_mild = lfptrials_mild.shape[2]
        ntrials_moderate = lfptrials_mild.shape[2]

        ntrials = np.min([ntrials_normal, ntrials_mild, ntrials_moderate])

        lfptrials_normal = lfptrials_normal[:, :, 0:ntrials]
        lfptrials_mild = lfptrials_mild[:, :, 0:ntrials]
        lfptrials_moderate = lfptrials_mild[:, :, 0:ntrials]
    ...
    if lfptrials_moderate is not None:
        ciCOH_moderate = calcciCOH_from_lfptrials(lfptrials_moderate)
        ciCOH['moderate'] = ciCOH_moderate

        setup['ntemp_moderate'] = lfptrials_moderate.shape[1]
