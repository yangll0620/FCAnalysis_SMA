function m0_segSKTData_extract_Jo()
    %% extract the SKT Data
    %
    %	Inputs:
    %
    %		xlsxfile_master
    ...
    server_NHP = server_NHP_extract(animal);
    ...
    for i = 24:nrecords
        ...
        if exist(fullfile(savefolder, savefilename), 'file') == 2
            clear outputfoldername dateofexp tdtbk pdcond savefilename
            continue;
        end
        ...
        if ~exist(onedaypath, 'dir')
            clear outputfoldername dateofexp tdtbk pdcond savefilename onedaypath
            continue;
        end
