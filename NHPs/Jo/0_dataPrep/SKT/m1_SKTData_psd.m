...
%%
% lfp_normal: nchns * ntemp * ntrials
...
% averaged across channels lfp_trials: ntemp * ntrials
lfp_trials = squeeze(mean(lfps, 1));

psd_alltrials = [];
for ti = 1: size(lfp_trials,2)
    lfp = lfp_trials(:, ti);
    ...
    psd_alltrials = cat(2, psd_alltrials, psd);
    
    clear psd lfp
end
psd_alltrials(isnan(psd_alltrials)) = 0;
psd_normal = mean(psd_alltrials, 2);
clear psd_alltrials lfp_trials lfps
...
psd_alltrials(isnan(psd_alltrials)) = 0;
psd_moderate = mean(psd_alltrials, 2);
clear psd_alltrials lfp_trials lfps
...
nchns = size(lfp_normal, 1);

for chni = 1: nchns
    
    % psd_normal
    lfp_trials = squeeze(lfp_normal(chni, :, :));
    psd_alltrials = [];
    for ti = 1: size(lfp_trials,2)
        lfp = lfp_trials(:, ti);
        %
    ...
    % psd_mild
    lfp_trials = squeeze(lfp_mild(chni, :, :));
    psd_alltrials = [];
    for ti = 1: size(lfp_trials,2)
        lfp = lfp_trials(:, ti);
        %
    ...
    % psd_normal
    lfp_trials = squeeze(lfp_moderate(chni, :, :));
    psd_alltrials = [];
    for ti = 1: size(lfp_trials,2)
        lfp = lfp_trials(:, ti);
        %
