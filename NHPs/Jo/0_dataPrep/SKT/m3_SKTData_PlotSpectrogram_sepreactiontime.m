function m3_SKTData_PlotSpectrogram_sepreactiontime()
...
for i = 1 : length(cond_cell)
    ...
    [lfptrials_larger, lfptrials_lower, fs, T_chnsarea] = lfp_goodTrials_align2_sepReaction(files, align2, tdur_trial, t_minmax_reach, t_minmax_return);
    ...
    % for lfptrials_larger
    lfptrials_larger = lfptrials_larger(mask_usedChns, :, :);
    plot_spectrogram_acrossTrials(lfptrials_larger, T_chnsarea, tdur_trial, fs, animal, pdcond, align2, savefolder, savefig_format, 'larger');
    saveas(gcf, fullfile(savefolder, [animal '_' pdcond '_larger']), savefig_format);
    close gcf
    
    % for lfptrials_lower
    lfptrials_lower = lfptrials_lower(mask_usedChns, :, :);
    plot_spectrogram_acrossTrials(lfptrials_lower, T_chnsarea, tdur_trial, fs, animal, pdcond, align2, savefolder, savefig_format, 'lower');
    saveas(gcf, fullfile(savefolder, [animal '_' pdcond '_lower']), savefig_format);
    close gcf
...
for idxGi = 1 : length(idxGroups)
    ...
    for idxi = 1 : length(idxs)
        ...
        title(ax, [animal ' ' pdcond ':' brainarea ', ntrials = ' num2str(ntrials) '-' largerlowername])
        ...
        title(ax_sep, [animal ' ' pdcond ':' brainarea ', ntrials = ' num2str(ntrials) '-' largerlowername]);
        savefile_sep = fullfile(savefolder, [animal '_sep_' pdcond '_' brainarea '_' largerlower]);
