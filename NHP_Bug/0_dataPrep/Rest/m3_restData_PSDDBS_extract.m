...
%  DBS PSD estimates for mild and normal individually
...
%% Codes Start Here
file_psdallsegs = fullfile(savefolder, [savefilename_prefix 'DBS_allsegs_normalmild.mat']);
...
if ~exist(file_psdallsegs, 'file') % not exist
    ...
    [psd_stn_allsegschns_mild, F_stn_pxx_mild] = psd_allsegs_fromfiles(files_mild, twin_pwelch, 'stn');
    [psd_gp_allsegschns_mild, F_gp_pxx_mild] = psd_allsegs_fromfiles(files_mild, twin_pwelch, 'gp');

    
    files_normal = dir(fullfile(inputfolder, '*_normal_*.mat'));
    [psd_stn_allsegschns_normal, F_stn_pxx_normal] = psd_allsegs_fromfiles(files_normal, twin_pwelch, 'stn');
    [psd_gp_allsegschns_normal, F_gp_pxx_normal] = psd_allsegs_fromfiles(files_normal, twin_pwelch, 'gp');
    

    if ~isequal(F_stn_pxx_mild, F_stn_pxx_normal, F_gp_pxx_mild, F_gp_pxx_normal)
        disp('F_pxx_normal not equal F_pxx_mild for stn or gp');
    else
        F_pxx = F_stn_pxx_normal;

        save(file_psdallsegs, 'psd_stn_allsegschns_mild', 'psd_stn_allsegschns_normal', 'psd_gp_allsegschns_mild', 'psd_gp_allsegschns_normal','F_pxx');
    end
else
    load(file_psdallsegs, 'psd_stn_allsegschns_mild', 'psd_stn_allsegschns_normal', 'psd_gp_allsegschns_mild', 'psd_gp_allsegschns_normal','F_pxx');
end


% plot PSD
plotPSD_comp(psd_stn_allsegschns_normal, psd_stn_allsegschns_mild, F_pxx, plotF_AOI, savefolder,'stn');
plotPSD_comp(psd_gp_allsegschns_normal, psd_gp_allsegschns_mild, F_pxx, plotF_AOI, savefolder,'gp');
