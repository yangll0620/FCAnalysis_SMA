...
for i = 1:length(lick_diff_ind)
    ...
    data.epocs.(LICK_EVENT).offset(i) = ...
        data.epocs.(LICK).offset(lick_diff_ind(i)-1);
...
% data.epocs.(LICK_EVENT).offset = data.epocs.(LICK).offset([lick_diff_ind-1; end])';
% data.epocs.(LICK_EVENT).data = ones(1, length(zonset))';
