...
% dates used by Ying
filesYingUsed = fullfile(codecorresParentfolder, 'm0_restData_uniqdatesYingUsed_extract', 'uniqFileYingUsed_Pinky.mat');
...
load(filesYingUsed, 'uniqDateBlocks');
...
for i = 1: length(uniqDateBlocks)
    
    dateblockstr = convertStringsToChars(uniqDateBlocks(i));
    ...
    % file not exist
    if(~exist(allsyncfile,'file'))
        disp([allsyncfile ' does not exist'])
        continue;
    end
    ...
    savefile = fullfile(savefolder, ['Pinky_cleanedRestData_'  cond '_'  dateblockstr '_.mat']);
...
%       dateblockstr: '20170918_5'
    ...
    % cond
    cond = parsePDCondition_Pinky(date_num);
end
