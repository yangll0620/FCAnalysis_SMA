function m0_SKTData_extract()
    ...
    %           a. abandon the file with max trial time > 5s
    %           b. trial length = max(each trial length) + t_bef + t_aft 
    %                   t_bef: the time before target on (default: t_bef = 1)
    %                   t_aft: the time after mouth (default: t_aft = 0.5)
    %           c. only remain the trials markedin both good reach and good return
    %
    %
    ...
    processedfolder_inroot2 = fullfile('/home', 'lingling', 'root2', 'Animals', animal, 'Recording', 'Processed', 'DataDatabase');
    ...
    for i = 1:nrecords
        ...
        %%% bipolar dbs %%%%
        lfptrial_dbs = diff(lfptrial_dbs, [], 1);
