function [psd_allsegs, F_pxx] = psd_dbs_allsegs_fromfiles(files, twin_pwelch, dbsarea)
%% extract dbs psd of all the segments from all files, each psd for each dbs contact
%
% Outputs:
%   
%       psd_allsegs: the PSD estimate of all the segments from the files  (nfs * nchns * nsegs)
%
%       F_pxx: the vector of frequencies (in hertz) at which the PSD is estimated (nfs * 1)
%
%       dbsarea: the dbs area to analysis ('stn', or 'gp')
...
for filei = 1 : nfiles
    ...
    for segi = 1: length(segsRemain)
        ...
        % extract the lfp data of segi in dbsarea
        eval(['lfp_oneseg = data_segments(segi).lfp_' dbsarea ';']);
