function fig5_freeze_SpeedExample(varargin)
%   
%   Usage:
...
% parse params
p = inputParser;
addParameter(p, 'pos_ifig', [150 150 500 250], @(x) assert(isvector(x) && isnumeric(x) && length(x)==4));
...
%% Code start Here
fig_freezeExample('pos_ifig', pos_ifig,...
...
if plotFreezeLines
    ...
    for tfi = 1 : size(tFreezePhases, 1)
        color = colors4freezeline{tfi};
        plot([tFreezePhases(tfi, 1) tFreezePhases(tfi, 1)], ys, [color '-'], 'LineWidth',0.5);
        plot([tFreezePhases(tfi, 2) tFreezePhases(tfi, 2)], ys, [color '-'], 'LineWidth',0.5);
    end
