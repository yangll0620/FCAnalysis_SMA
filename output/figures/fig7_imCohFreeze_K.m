...
w_deltax_colormap = 10; % y distance between two color map of animal K
...
ncols = nfTypes;
fig_height = h_textFreeze + h_colormap + (h_textFreNum + h_textFreLabel);
fig_width = w_textpair + w_colormap * ncols + w_deltax_colormap * (ncols-1)+ w_textColorbar;
...
h_outer_top = 0;
h_outer_bottom = 0;
h_inner_top = h_textFreeze;
h_inner_bottom = h_textFreLabel + h_textFreNum;
for coli = 1 : ncols
    freeType = freezeTypes {coli};
    
    ciCohPhasefile = ['Kitty-Freeze-' freeType '_ciCoh_ntrial84_moderate.mat'];
    load(fullfile(input_folder, ciCohPhasefile), 'ciCohs', 'T_chnsarea', 'f_selected', 'psedociCohs');
    ...
    w_outer_left = w_textpair + (w_colormap + w_deltax_colormap)*(ncols-1);
    ...
    clear w_outer_left w_outer_right w_outer_diff w_inner_left w_inner_right
    clear freeType ciCohPhasefile sigciCoh sigciCoh_flatten chnPairNames
    clear show_pairname show_colorbar
    clear('ciCohs', 'T_chnsarea', 'f_selected', 'psedociCohs');
...
for coli = 1 : nrows
    ...
    h_outer_top = (coli - 1) * (h_textFreeze + h_colormap + h_deltay_colormap);
    h_inner_top = 0;
    h_inner_bottom = 0;
